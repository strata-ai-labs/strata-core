...
/// Convert a byte key to a hex string for Strata's string-key API.
...
pub struct StrataBenchDatabase {
    db: Strata,
}

impl StrataBenchDatabase {
    pub fn new(db: Strata) -> Self {
        Self { db }
    }
}

impl BenchDatabase for StrataBenchDatabase {
    type C<'db>
        = StrataBenchDatabaseConnection<'db>
    where
        Self: 'db;
    ...
    fn connect(&self) -> Self::C<'_> {
        StrataBenchDatabaseConnection { db: &self.db }
    }
}

pub struct StrataBenchDatabaseConnection<'a> {
    db: &'a Strata,
}

impl BenchDatabaseConnection for StrataBenchDatabaseConnection<'_> {
    type W<'db>
        = StrataBenchWriteTransaction<'db>
    where
        Self: 'db;
    type R<'db>
        = StrataBenchReadTransaction<'db>
    where
        Self: 'db;
    ...
    fn read_transaction(&self) -> Self::R<'_> {
        StrataBenchReadTransaction { db: self.db }
    }
}
...
pub struct StrataBenchReadTransaction<'a> {
    db: &'a Strata,
}

impl<'a> BenchReadTransaction for StrataBenchReadTransaction<'a> {
    type T<'txn>
        = StrataBenchReader<'a>
    where
        Self: 'txn;
    ...
    fn get_reader(&self) -> Self::T<'_> {
        StrataBenchReader { db: self.db }
    }
}

pub struct StrataBenchReader<'a> {
    db: &'a Strata,
}

impl BenchReader for StrataBenchReader<'_> {
    type Output<'out>
        = Vec<u8>
    where
        Self: 'out;
    ...
    fn get(&self, key: &[u8]) -> Option<Vec<u8>> {
        let hex_key = bytes_to_hex(key);
        match self.db.kv_get(&hex_key) {
            Ok(Some(Value::Bytes(b))) => Some(b),
            _ => None,
        }
    }

    fn range_from<'a>(&'a self, key: &'a [u8]) -> Self::Iterator<'a> {
        // Use kv_scan (operates in the current branch's namespace) rather than
        // manually building a Key with Default::default() which would point at
        // a random branch namespace and return an empty scan.
        let hex_key = bytes_to_hex(key);
        let rows = self
            .db
            .kv_scan(Some(&hex_key), Some(SCAN_LEN as u64))
            .unwrap_or_default();
        StrataBenchIterator {
            rows: rows.into_iter(),
        }
    ...
    fn len(&self) -> u64 {
        self.db.kv_count(None).unwrap_or(0)
    }
}

pub struct StrataBenchIterator {
    rows: std::vec::IntoIter<(String, Value)>,
}
...
// See strata-benchmarks repo for full comparison suite.
