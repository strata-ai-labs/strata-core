...
4. Vector history responses are arrays of version entries, not `{ count, items }`.
5. `event list --limit 2` can return a truncated result with
   `has_more == false` and `cursor == null`; event list does not currently
   expose a usable continuation cursor.
6. Event reverse ranges reverse a bounded forward interval, for example
   `event range 0 --end-seq 3 --direction reverse`; `event range 2 --direction
   reverse` does not mean "walk backward from sequence 2".
7. Event chain verification reports `is_valid`, not `valid`.
8. Graph neighbor page items wrap neighbor data under `node` and edge data
    under `edge`; neighbor ids are `item.node.node_id`.
9. Arrow graph export treats the requested output path as a stem and writes
    separate node and edge files, returning their paths in the response. The
    exact requested path may not exist.
10. CLI JSON error envelopes expose registry-style `retry_policy` rather than a
    boolean `retryable`.
