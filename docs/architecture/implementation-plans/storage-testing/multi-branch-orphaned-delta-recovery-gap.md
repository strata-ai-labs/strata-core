...
**Mechanism.** Before a checkpoint records a snapshot, `non_seeded_branch_has_durable_base(branch_catalog, seeded_branch_id)` scans the active branches; if any branch other than the recovery-seeded (initial) branch has `owned_table_count() > 0` (a durable table-manifest base), the checkpoint returns `Deferred` (`LifecycleCheckpointStatus::DeferredNonSeededBranchBase`) instead of publishing. It is enforced on **both** drain paths — the synchronous runtime collector (`checkpoint_durable_runtime_with_budget`, used by the explicit and synchronous-maintenance paths) and the background builder (`start_next_background_checkpoint_maintenance`) — so neither can record the unrecoverable snapshot.
