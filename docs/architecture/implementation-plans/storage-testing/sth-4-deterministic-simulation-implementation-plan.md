...
Status: **4b + 4c + 4d implemented (2026-06-18)**; 4a descoped. Class 9 **stays open**. 4c's DST surfaced a silent durability bug (publish fault during checkpoint+flush) — now **fixed** (see the finding doc's Resolution). With that fixed the soak progressed and surfaced a *separate, pre-existing* power-loss `Gap` at seed 155 (`sth-4-finding-splitrename-power-loss-gap.md`); class 9 closes when that is fixed and the fault-simulation soak runs clean end-to-end.
...
**Blocking class 9 (the DST did its job — twice):**
...
- With that fixed, the soak progressed and surfaced a **separate, pre-existing
  power-loss `Gap` at seed 155** (SplitRename/Standard, no injected fault — independent
  of the publish-fault fix): `sth-4-finding-splitrename-power-loss-gap.md`.
- **Class 9 closes when the seed-155 bug is fixed (its own slice) and the
  fault-simulation soak runs clean end-to-end.**
