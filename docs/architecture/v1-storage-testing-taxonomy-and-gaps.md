...
| 5. Error-path sweeps | 🟡 Partial | Seams exist: `fault-injection` feature; 8 backend I/O fault steps (`LocalFsPublishStep`: TemporaryCreate/Write/Sync/FinalPublish/ParentSync; `LocalFsDeleteStep`: BeforeRemoval/Removal/ParentSync). But tests drive 19 *enumerated* windows with literal `NonZeroU64::new(N)`, not "fail op N, sweep N." No disk-full/ENOSPC, no OOM/budget-exhaustion injection |
