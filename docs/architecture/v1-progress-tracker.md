...
Status: M3TC2G complete; tracker active
...
| `M3` | Storage-next backend, layout, format, and durable services | In Progress | In Progress | In Progress | `M3TC2G` closes the WAL review gaps; decide whether `M3TB2` service-level WAL fuzzing is still needed, then continue with `M3E3` snapshot/checkpoint services. |
...
| `M3TC` | Durable fault-window tests | Test | In Progress | none | `M3TC1` covers durable-publish fault windows; `M3TC2A` adds WAL construction, capability, and listing hardening; `M3TC2B` adds append, exact-boundary, rotation, and property-model coverage; `M3TC2C` adds durability policy and sync-failure coverage; `M3TC2D` adds the WAL corruption matrix; `M3TC2E` adds backend fault-window and partial-visibility coverage; `M3TC2F` adds WAL retention/deletion/reopen coverage; `M3TC2G` closes read/watermark and review-discovered edge-case gaps. Decide optional `M3TB2` before later snapshot and quarantine service windows. |
...
1. `M3TB2`: decide whether service-level WAL fuzzing remains useful after the module-local WAL suite.
