...
| G8 | Compaction merge ~270 MB/s (memory-bound; per-row policy/encode overhead) | Slice 4 A/B measurements | — (profile-driven) | **BS3** |
...
| G10 | Admission thresholds tight + abrupt vs RocksDB (L0 urgent/block 8/16 vs slowdown/stop 20/36) | `compaction.rs:33-47`, config comment | generous soft/hard split + stateful rate ramp | **BS3** |
