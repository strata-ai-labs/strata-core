...
`crates/inference/vendor/llama.cpp/CMakeLists.txt` prerequisite. The
post-closeout STAB1 stabilization fixed the 10 shutdown/fault-injection and
branch cleanup/classification failures, and `cargo test -p strata-engine` now
passes. No temporary engine-consolidation compatibility shell survives.
