...
use strata_engine_next::api::CommitOutcome;
...
fn invalid_inputs_return_engine_errors() {
    ...
    let empty = strata_engine_next::KvValue::new(Vec::new());
...
fn kv_batch_validation_is_stable() {
    ...
    let error = kv
        .put_batch(std::iter::empty::<(
            strata_engine_next::KvKey,
            strata_engine_next::KvValue,
        )>())
    ...
    let error = kv
        .delete_batch(std::iter::empty::<strata_engine_next::KvKey>())
