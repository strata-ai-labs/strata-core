...
fn exercise_bulk_refusals(mut database: Database) {
    ...
    graph
        .define_object_type(
            &name,
            strata_engine_next::GraphObjectTypeDef::new(
                strata_engine_next::GraphTypeName::new("Document").expect("type name"),
                std::iter::empty::<(String, strata_engine_next::GraphPropertyDef)>(),
            )
            .expect("object type"),
        )
        .expect("type defined");
    ...
    let typed = vec![(
        node_id("typed"),
        GraphNodeData::default().with_object_type(
            strata_engine_next::GraphTypeName::new("Undeclared").expect("type name"),
        ),
    )];
    let error = graph
