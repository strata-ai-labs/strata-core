...
fn row_class_assignments_match_registry_contract() {
    assert_eq!(
        strata_engine::testkit::row_class_storage_id_for_test("kv").expect("known class"),
        0x20
    );
    assert_eq!(
        strata_engine::testkit::row_class_storage_id_for_test("json").expect("known class"),
        0x22
    );
    assert_eq!(
        strata_engine::testkit::row_class_storage_id_for_test("json-index").expect("known class"),
        0x24
    );
    assert_eq!(
        strata_engine::testkit::row_class_storage_id_for_test("branch").expect("known class"),
        0x30
    );
    assert_eq!(
        strata_engine::testkit::row_class_storage_id_for_test("space-control")
            .expect("known class"),
        0x31
    );
    assert_eq!(
        strata_engine::testkit::row_class_storage_id_for_test("registry").expect("known class"),
        0x32
    );
    assert_eq!(
        strata_engine::testkit::row_class_storage_id_for_test("identity").expect("known class"),
        0x34
    );
}
...
fn assert_engine_error_is_boundary_shaped(error: &strata_engine::EngineError) {
