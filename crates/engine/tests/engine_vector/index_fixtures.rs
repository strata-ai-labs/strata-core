...
impl Small2dFixture {
    fn seed(vectors: &mut strata_engine::VectorService<'_>) -> Self {
...
struct DuplicateRevisionFixture {
    collection: VectorCollectionName,
    after_user_1_insert: strata_core::Timestamp,
    after_user_2_insert: strata_core::Timestamp,
}
...
impl DuplicateRevisionFixture {
    ...
    const fn after_user_1_insert(&self) -> strata_core::Timestamp {
        self.after_user_1_insert
    }

    const fn after_user_2_insert(&self) -> strata_core::Timestamp {
    ...
    fn assert_timestamp_query(
        &self,
        vectors: &mut strata_engine::VectorService<'_>,
        timestamp: strata_core::Timestamp,
        expected_keys: &[&str],
    ) {
        let exact = vectors
