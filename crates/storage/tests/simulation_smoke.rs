...
fn simulation_holds_oracle_and_liveness_across_seeds() {
    ...
    let outcome = strata_storage::testkit::run_simulation_harness(&root, case_limit.or(Some(16)))
...
fn simulation_soak_deepens_across_many_seeds() {
    ...
    let outcome =
        strata_storage::testkit::run_simulation_harness(tempdir.path(), case_limit.or(Some(2_000)))
