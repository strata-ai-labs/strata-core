...
fn lifecycle_recovery_contract_exercises_storage_recovery_paths() {
    let outcome = strata_storage::testkit::check_lifecycle_recovery_contract(b"recovery-seed")
...
fn lifecycle_bootstrap_contract_exercises_commit_bootstrap_paths() {
    let outcome = strata_storage::testkit::check_lifecycle_bootstrap_contract(b"bootstrap-seed")
...
fn generated_recovery_empty_checkpoint_tail_and_lossy_routes_are_input_driven() {
    let outcome = strata_storage::testkit::check_lifecycle_generated_script_contract(
...
fn generated_recovery_corrupt_manifest_snapshot_wal_and_table_are_typed() {
    let corrupt_log = strata_storage::testkit::check_lifecycle_fault_contract(b"fault-corrupt-log")
        .expect("corrupt-log fault contract");
    let partial_log =
        strata_storage::testkit::check_lifecycle_fault_contract(b"fault-partial-log-strict")
...
fn generated_bootstrap_catches_allocator_timestamp_and_visible_facts() {
    let outcome = strata_storage::testkit::check_lifecycle_bootstrap_contract(b"bootstrap-facts")
...
fn generated_bootstrap_rejects_timeline_mismatch() {
    let outcome = strata_storage::testkit::check_lifecycle_bootstrap_contract(b"timeline-mismatch")
        .expect("bootstrap contract");
...
fn generated_bootstrap_reconciles_unresolved_durable_gate() {
    let replay_failed =
        strata_storage::testkit::check_lifecycle_fault_contract(b"fault-replay-failed")
            .expect("replay-failed fault contract");
    let replay_visible =
        strata_storage::testkit::check_lifecycle_fault_contract(b"fault-replay-visible")
...
fn generated_recovery_health_matches_fault_family_model() {
    let outcome =
        strata_storage::testkit::check_lifecycle_generated_script_contract(b"health-model")
            .expect("generated lifecycle contract");
