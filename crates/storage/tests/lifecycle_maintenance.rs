...
fn lifecycle_maintenance_contract_covers_executor_categories() {
    let outcome =
        strata_storage::testkit::check_lifecycle_maintenance_contract(b"maintenance-executor-seed")
...
fn lifecycle_executor_enqueue_and_coalesce_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_maintenance_contract(
...
fn lifecycle_maintenance_contract_covers_flush_categories() {
    let outcome = strata_storage::testkit::check_lifecycle_flush_contract(b"flush-contract-seed")
...
fn lifecycle_flush_cache_and_durable_integration() {
    let outcome =
        strata_storage::testkit::check_lifecycle_flush_contract(b"flush-success-integration")
            .expect("flush contract");
...
fn lifecycle_flush_failure_integration() {
    let outcome =
        strata_storage::testkit::check_lifecycle_flush_contract(b"flush-failure-integration")
            .expect("flush contract");
...
fn lifecycle_flush_retry_and_read_parity_integration() {
    let outcome =
        strata_storage::testkit::check_lifecycle_flush_contract(b"flush-retry-integration")
            .expect("flush contract");
...
fn lifecycle_checkpoint_publication_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_checkpoint_contract(
...
fn lifecycle_table_rewrite_compaction_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_table_rewrite_contract(
...
fn lifecycle_retention_proof_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_retention_contract(b"retention-proof");
...
fn lifecycle_retention_durable_runner_drives_real_backend_end_to_end() {
    ...
    let outcome =
        strata_storage::testkit::run_localfs_lifecycle_retention_runner_harness(tempdir.path())
...
fn lifecycle_snapshot_pruning_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_retention_contract(b"snapshot-pruning");
...
fn lifecycle_table_retention_delegation_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_retention_contract(b"table-delegation");
...
fn generated_table_object_reachability_covers_ordering_and_safety_categories() {
    let outcome =
        strata_storage::testkit::check_lifecycle_retention_contract(b"table-reachability");
...
fn lifecycle_retention_blocks_unsafe_recovery_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_retention_contract(b"retention-blocked");
...
fn lifecycle_table_retention_delegates_to_quarantine_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_retention_contract(b"table-quarantine");
...
fn lifecycle_snapshot_pruning_delete_failure_integration() {
    let outcome =
        strata_storage::testkit::check_lifecycle_retention_contract(b"snapshot-delete-failure");
...
fn lifecycle_quarantine_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"quarantine-flow")
...
fn lifecycle_close_contract_covers_shutdown_categories() {
    let outcome =
        strata_storage::testkit::check_lifecycle_close_contract(b"close-shutdown-categories")
...
fn lifecycle_close_contract_routes_are_input_derived() {
    let wal = strata_storage::testkit::check_lifecycle_close_contract(b"close-wal-sync")
        .expect("WAL close route");
    let retry = strata_storage::testkit::check_lifecycle_close_contract(b"close-retry")
...
fn lifecycle_purge_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"purge-flow")
        .expect("quarantine contract");
...
fn lifecycle_repair_reconciliation_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"repair-flow")
        .expect("quarantine contract");
...
fn lifecycle_reclaim_blocks_unsafe_recovery_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"blocked-reclaim")
        .expect("quarantine contract");
...
fn lifecycle_cache_reclaim_unsupported_integration() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"cache-reclaim")
        .expect("quarantine contract");
...
fn lifecycle_quarantine_then_purge_round_trip() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"round-trip")
        .expect("quarantine contract");
...
fn lifecycle_quarantine_publish_failure_surfaces_health_debt() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"publish-faults")
        .expect("quarantine contract");
...
fn lifecycle_quarantine_generated_bytes_influence_routes() {
    let staging = strata_storage::testkit::check_lifecycle_quarantine_contract(&[
        0, 1, 2, 3, 4, 0, 1, 8, 1, 0, 0, 0, 0, 0,
    ])
    .expect("staging contract");
    let purge = strata_storage::testkit::check_lifecycle_quarantine_contract(&[
...
fn lifecycle_retention_generated_bytes_influence_decision_counts() {
    let low_retain = strata_storage::testkit::check_lifecycle_retention_contract(&[0, 0])
        .expect("low retain contract");
    let high_retain = strata_storage::testkit::check_lifecycle_retention_contract(&[0, 2])
...
fn lifecycle_generated_integration_runs_default_mode_script() {
    let outcome =
        strata_storage::testkit::check_lifecycle_generated_script_contract(b"default-script")
...
fn lifecycle_generated_integration_runs_durable_mode_script() {
    let outcome =
        strata_storage::testkit::check_lifecycle_generated_script_contract(b"durable-script")
            .expect("generated lifecycle contract");
...
fn lifecycle_generated_integration_runs_reclaim_close_script() {
    let outcome =
        strata_storage::testkit::check_lifecycle_generated_script_contract(b"reclaim-close-script")
            .expect("generated lifecycle contract");
...
fn lifecycle_fault_integration_covers_all_phase_families() {
    ...
    let flush = strata_storage::testkit::check_lifecycle_fault_contract(b"fault-flush-orphan")
...
fn lifecycle_crash_integration_reports_case_counts() {
    ...
    let snapshot =
        strata_storage::testkit::check_lifecycle_crash_contract(b"crash-orphan-snapshot")
...
fn generated_maintenance_model_matches_enqueue_coalesce_run_cancel_drain() {
    let outcome = strata_storage::testkit::check_lifecycle_maintenance_contract(b"model");
...
fn generated_maintenance_queue_full_and_admission_rejections_are_typed() {
    let outcome = strata_storage::testkit::check_lifecycle_maintenance_contract(b"queue");
...
fn generated_flush_preserves_read_parity_and_candidate_watermark() {
    let outcome = strata_storage::testkit::check_lifecycle_flush_contract(b"flush-parity")
        .expect("flush contract");
...
fn generated_flush_publication_failure_keeps_branch_state_safe() {
    let outcome = strata_storage::testkit::check_lifecycle_flush_contract(b"flush-failure")
        .expect("flush contract");
...
fn generated_checkpoint_preserves_row_visibility_and_tail_replay() {
    let outcome = strata_storage::testkit::check_lifecycle_checkpoint_contract(b"checkpoint-tail")
        .expect("checkpoint contract");
...
fn generated_checkpoint_truncation_never_removes_uncovered_wal_records() {
    let outcome = strata_storage::testkit::check_lifecycle_generated_script_contract(b"truncation")
        .expect("generated lifecycle contract");
...
fn generated_table_rewrite_preserves_reads_after_compaction() {
    let outcome = strata_storage::testkit::check_lifecycle_table_rewrite_contract(b"compaction")
        .expect("table rewrite contract");
...
fn generated_row_pruning_covers_retained_history_boundaries() {
    let outcome = strata_storage::testkit::check_lifecycle_row_pruning_contract(b"row-pruning")
...
fn generated_materialization_preserves_child_precedence() {
    let outcome =
        strata_storage::testkit::check_lifecycle_table_rewrite_contract(b"materialization")
            .expect("table rewrite contract");
...
fn generated_storage_pressure_suggestions_are_model_consistent() {
    let outcome = strata_storage::testkit::check_lifecycle_table_rewrite_contract(b"pressure")
        .expect("table rewrite contract");
...
fn generated_retention_proof_blocks_unsafe_recovery_health() {
    let outcome = strata_storage::testkit::check_lifecycle_retention_contract(b"blocked")
...
fn generated_retention_never_deletes_reachable_tables_or_live_snapshots() {
    let outcome =
        strata_storage::testkit::check_lifecycle_generated_script_contract(b"safe-delete")
            .expect("generated lifecycle contract");
...
fn generated_snapshot_pruning_retains_live_and_newest_snapshots() {
    let outcome = strata_storage::testkit::check_lifecycle_retention_contract(b"snapshots")
        .expect("retention contract");
...
fn generated_quarantine_happens_before_purge() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"quarantine-purge")
        .expect("quarantine contract");
...
fn generated_purge_requires_fresh_inventory_proof() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"stale-proof")
        .expect("quarantine contract");
...
fn generated_repair_reports_inconclusive_without_mutating_state() {
    let outcome = strata_storage::testkit::check_lifecycle_quarantine_contract(b"repair")
        .expect("quarantine contract");
...
fn generated_close_blocks_new_commits_and_ordinary_maintenance() {
    let outcome = strata_storage::testkit::check_lifecycle_close_contract(b"close-block")
        .expect("close contract");
...
fn generated_close_retry_and_double_close_match_model() {
    let outcome = strata_storage::testkit::check_lifecycle_close_contract(b"close-retry")
        .expect("close contract");
...
fn generated_close_faults_preserve_health_debt() {
    let wal = strata_storage::testkit::check_lifecycle_fault_contract(b"fault-close-wal-sync")
        .expect("wal fault contract");
    let manifest = strata_storage::testkit::check_lifecycle_fault_contract(b"fault-close-manifest")
