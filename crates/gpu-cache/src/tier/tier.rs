...
use crate::tier::page_table::{Epoch, EvictOutcome, PageId, PageTable};
...
pub struct TierStats {
    ...
    /// Evictions from this handle's view: fence-gated last releases plus
    /// shared-reference releases (HT-11).
    pub evictions: u64,
...
/// Union state shared by every handle of one tier family (HT-11). Device
/// structure regions (adjacency rows, validity) are global device state,
/// so their host mirrors must be global too: unlinking a dying slot needs
/// every handle's links, not just the releaser's. The page-id clock is
/// shared so ids stay unique across handles after a fork — id-keyed
/// structures (edge lists, waiting links) remain unambiguous once branches
/// diverge.
struct TierUnion {
    /// Next page id, unique across the whole handle family.
    next_page_id: u64,
    /// Per-slot edge lists of the resident page (T2 page ids).
    ...
    /// Registrations are removed when their slot uninstalls, so a present
    /// registration always names a live occupant.
    waiting_edges: std::collections::HashMap<PageId, Vec<u32>>,
}

/// One slot's device adjacency row from the union mirror (`u32::MAX` pads
/// empty entries).
fn adjacency_row_bytes(union: &TierUnion, slot: u32, degree: usize) -> Vec<u8> {
    let mut row = vec![0xFFu8; degree * 4];
    for (j, neighbor) in union.slot_adj[slot as usize].iter().enumerate() {
...
pub struct Tier<B: DeviceBackend, S: PageStore> {
    ...
    /// Structure mirrors + page-id clock, shared across the handle family
    /// (HT-11). Same guard discipline as `backend`: locked per operation,
    /// never held across a device call.
    union: Arc<Mutex<TierUnion>>,
}
...
impl<B: DeviceBackend, S: PageStore> Tier<B, S> {
    ...
    pub fn open(mut backend: B, mut store: S, config: TierConfig) -> Result<Self, GpuError> {
        ...
        Ok(Self {
            ...
            union: Arc::new(Mutex::new(TierUnion {
                next_page_id,
    ...
    /// Forks this handle (HT-11, design §9): the child shares the device
    /// arena, slot pool, and union structures, starts with this handle's
    /// *activated* working set — metadata only, no page copies — and takes
    /// `store` as its own branch of record. Promotion, write-behind, and
    /// stats start fresh.
    ///
    /// Contracts (v0.5):
    /// - The parent must be flushed: refuses
    ///   `failed_precondition.tier.fork_unflushed` while write-behind
    ///   entries are queued, so a forked handle never references pages that
    ///   are not durable on the parent's branch.
    /// - `store` must be a fork of this handle's store of record with the
    ///   same geometry manifest (engine branch fork lands in HT-11c;
    ///   `InMemoryStore::fork` serves the machinery tests).
    /// - One thread drives a handle family; fork at a step boundary.
    /// - Selections run over the union working set (device arrays are
    ///   shared) — scope queries with tag filters where strict isolation
    ///   matters. Readback paths (`topk_pages`, `page_of_slot`) name only
    ///   this handle's pages.
    pub fn fork(&self, mut store: S) -> Result<Self, GpuError> {
        ...
        match store.load_manifest()? {
            ...
            None => {
                return Err(GpuError::InvalidConfig {
                    detail: "forked store carries no tier manifest; \
                             fork the parent's store of record"
                        .to_owned(),
                });
            }
        }
    ...
    pub fn append(&mut self, blob: &PageBlob) -> Result<PageId, GpuError> {
        ...
        let page_id = {
            let mut union = self.union.lock().expect("union lock poisoned");
            let id = PageId(union.next_page_id);
            union.next_page_id += 1;
            id
        };
    ...
    fn install_page(&mut self, page_id: PageId, blob: &PageBlob, dirty: bool) {
        ...
        // Structure bookkeeping first (union mirrors + neighbor rows). The
        // union guard is scoped to this block; device writes use the row
        // snapshots taken inside it. Neighbor slots may belong to other
        // handles: device-level links are sound for the union (slots are
        // global and pages immutable) even when the neighbor's page id is
        // not in this handle's table.
        let index = slot as usize;
        let degree = usize::from(self.config.adjacency_degree);
        let mut prefetch = Vec::new();
        let mut neighbor_bumps = vec![];
        let mut adjacency_rows: Vec<(u32, Vec<u8>)> = Vec::new();
        {
            let union_arc = Arc::clone(&self.union);
            let mut union = union_arc.lock().expect("union lock poisoned");
            union.slot_edges[index].clone_from(&blob.edges);
            ...
            for edge in &blob.edges {
                if let Some(neighbor) = self.table.slot_of(*edge) {
                    ...
                    union.waiting_edges.entry(*edge).or_default().push(slot);
                    // Edge-driven prefetch (HT-4): graph neighbors of a
                    // touched page are promotion candidates the moment it
                    // is placed.
                    prefetch.push(*edge);
                }
            }
            // Slots waiting for *this* page link up now. A registration
            // present in the union always names a live occupant (they are
            // removed at uninstall), possibly another handle's.
            if let Some(waiters) = union.waiting_edges.remove(&page_id) {
            ...
            for neighbor in linked {
                if union.slot_adj[index].len() < degree {
                    union.slot_adj[index].push(neighbor);
                    neighbor_bumps.push((slot, 1));
                }
                let neighbor_index = neighbor as usize;
                if union.slot_adj[neighbor_index].len() < degree {
                    union.slot_adj[neighbor_index].push(slot);
                    neighbor_bumps.push((neighbor, 1));
                    dirty_rows.push(neighbor);
                }
            }
            for row in dirty_rows {
                adjacency_rows.push((row, adjacency_row_bytes(&union, row, degree)));
            }
        }
        for (bumped, delta) in neighbor_bumps {
            // Per-handle recency/edge-awareness state: foreign slots are a
            // no-op here, and the other handles' counts drift until their
            // own link/unlink actions (documented v0.5 policy input).
            self.table.add_resident_neighbor(bumped, delta);
        }
        for edge in prefetch {
        ...
        let copied = (|| {
            ...
            for (row, bytes) in &adjacency_rows {
                self.device().copy_in(
                    Region::Adjacency,
                    u64::from(*row) * (degree as u64) * 4,
                    bytes,
                )?;
            }
    ...
    /// Unlinks a slot from the *union* adjacency structures: every
    /// neighbor row loses the entry (mirror + device), waiting
    /// registrations drop. Called only when this handle owns the slot's
    /// last reference (failed installs, last-gated evictions) — a shared
    /// release leaves the structures intact for the surviving handles.
    /// Idempotent.
    fn uninstall_structures(&mut self, slot: u32) {
        let index = slot as usize;
        let degree = usize::from(self.config.adjacency_degree);
        let mut neighbor_drops = Vec::new();
        let mut rewrites: Vec<(u32, Vec<u8>)> = Vec::new();
        {
            let union_arc = Arc::clone(&self.union);
            let mut union = union_arc.lock().expect("union lock poisoned");
            for edge in std::mem::take(&mut union.slot_edges[index]) {
                if let Some(waiters) = union.waiting_edges.get_mut(&edge) {
                    waiters.retain(|&w| w != slot);
                    if waiters.is_empty() {
                        union.waiting_edges.remove(&edge);
                    }
                }
            }
            let neighbors = std::mem::take(&mut union.slot_adj[index]);
            for neighbor in neighbors {
                let neighbor_index = neighbor as usize;
                let before = union.slot_adj[neighbor_index].len();
                union.slot_adj[neighbor_index].retain(|&n| n != slot);
                if union.slot_adj[neighbor_index].len() != before {
                    neighbor_drops.push(neighbor);
                    rewrites.push((neighbor, adjacency_row_bytes(&union, neighbor, degree)));
                }
            }
        }
        for neighbor in neighbor_drops {
            self.table.add_resident_neighbor(neighbor, -1);
        }
        for (row, bytes) in rewrites {
            // Best-effort, as before the union rework: a failed unlink
            // write degrades (the row is rewritten on the neighbor's next
            // link/unlink); eviction must not stall on it.
            let _ = self.device().copy_in(
                Region::Adjacency,
                u64::from(row) * (degree as u64) * 4,
                &bytes,
            );
    ...
    fn ensure_headroom(&mut self, wanted: usize) {
        loop {
            ...
            // HT-11: a slot shared with other handles releases this
            // handle's reference only — no device writes, the union keeps
            // it valid and selectable, and it frees when the last handle
            // evicts it. It yields no headroom here, but the loop still
            // terminates: every round removes the victim from this
            // handle's candidates. (Check-then-act on the count is sound
            // under the v0.5 contract that one thread drives a family.)
            if self.table.shared_references(victim) > 1 {
                match self.table.evict(victim) {
                    Ok(outcome) => debug_assert_eq!(outcome, EvictOutcome::SharedRelease),
                    Err(_) => return,
                }
                self.stats.evictions += 1;
                continue;
            }
            // Last reference: unselectable on the device first, then host
            // bookkeeping (full union unlink + fence-gated reuse).
            if self
                .device()
...
/// Dropping a handle releases its references to slots it shares with
/// surviving handles, so they can evict and reuse them. Exclusive slots
/// and pending reuse gates stay allocated until the family tears down —
/// freeing them safely needs a fence this handle can no longer wait on
/// (v0.5; orphan-gate handoff is the follow-up).
impl<B: DeviceBackend, S: PageStore> Drop for Tier<B, S> {
    fn drop(&mut self) {
        self.table.drop_shared_references();
    }
}
