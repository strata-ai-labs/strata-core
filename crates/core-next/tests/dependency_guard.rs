//! Dependency boundary tests for core-next.
...
const PACKAGE_NAME: &str = "strata-core-next";
...
fn core_next_has_no_strata_crate_dependencies() {
    ...
    let package = packages
        ...
        .expect("core-next package should exist in cargo metadata");
    let package_id = package
        .get("id")
        .and_then(Value::as_str)
        .expect("core-next package should have an id");
    ...
    assert!(
        ...
        "core-next must not depend on any Strata crate. Manifest violations: {manifest_violations:?}. Graph violations: {graph_violations:?}"
    );
}
...
fn workspace_root() -> PathBuf {
    ...
    manifest_dir
        ...
        .expect("core-next should live under crates/core-next")
        .to_path_buf()
}
...
mod tests {
    ...
    fn graph_guard_rejects_direct_workspace_dependencies() {
        ...
        assert_eq!(
            graph_strata_dependency_violations("core", &package_by_id, &node_by_id),
            vec!["strata-core-next -> internal-tools"]
        );
    }
    ...
    fn graph_guard_rejects_transitive_strata_dependencies() {
        ...
        assert_eq!(
            graph_strata_dependency_violations("core", &package_by_id, &node_by_id),
            vec!["strata-core-next -> external -> strata-storage"]
        );
    }
