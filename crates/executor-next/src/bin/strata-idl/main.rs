...
fn main() -> ExitCode {
    let Some(command) = env::args().nth(1) else {
        eprintln!(
            "usage: cargo run -p strata-executor-next --features idl-tooling --bin strata-idl -- <generate|check|generate-cli|check-cli>"
        );
    ...
    let result = match command.as_str() {
        ...
        "generate-cli" => strata_executor_next::idl_tooling::generate_cli(&root),
        "check-cli" => strata_executor_next::idl_tooling::check_cli(&root),
        _ => {
            eprintln!(
