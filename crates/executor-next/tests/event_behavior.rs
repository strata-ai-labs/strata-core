...
fn run_event_command_suite(executor: &mut Executor) {
    ...
    assert_eq!(
        ...
        strata_executor_next::EventChainVerification::new(true, 0, None, None)
    );
...
) -> Vec<strata_executor_next::EventBatchAppendItemResult> {
    match executor
        .execute(Command::EventBatchAppend {
            branch: branch.map(str::to_owned),
            space: space.map(str::to_owned),
...
fn verify_chain(
    ...
    branch: Option<&str>,
    space: Option<&str>,
) -> strata_executor_next::EventChainVerification {
    match executor
