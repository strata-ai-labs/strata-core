...
fn assert_graph_executor_inherits_configured_database_default_branch(executor: &mut Executor) {
    ...
    assert_eq!(
        error.class(),
        strata_executor_next::ExecutorErrorClass::NotFound
    );
}
...
fn create_graph(executor: &mut Executor, graph: &str) -> strata_executor_next::GraphInfoData {
    match executor
        .execute(Command::GraphCreate {
            branch: None,
            space: None,
...
fn graph_create_in(
    ...
    space: Option<&str>,
    graph: &str,
) -> strata_executor_next::GraphInfoData {
    match executor
        .execute(Command::GraphCreate {
...
fn get_meta(executor: &mut Executor, graph: &str) -> Option<strata_executor_next::GraphInfoData> {
    match executor
...
fn get_edge(
    ...
    edge_type: &str,
    dst: &str,
) -> Option<strata_executor_next::GraphEdgeDataOutput> {
    match executor
        .execute(Command::GraphGetEdge {
            branch: None,
            space: None,
...
) -> Vec<strata_executor_next::GraphBatchItemResult> {
    match executor
