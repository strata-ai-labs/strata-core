...
{"data":{"cursor":null,"has_more":false,"items":["a","b"]},"type":"vector_key_page"}
```
...
```text
strata vector keys <collection> [--as-of <integer>] [--cursor <string>] [--limit <integer>] [--prefix <string>] [--branch <branch>] [--space <space>]
```
