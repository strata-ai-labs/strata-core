...
fn main() -> ExitCode {
    let Some(command) = env::args().nth(1) else {
        eprintln!(
            "usage: cargo run -p strata-executor --features idl-tooling --bin strata-idl -- <generate|check|generate-cli|check-cli|generate-docs|check-docs|verify-fixtures [--update]>"
        );
